(function(global) {
  ...
  var initWorker = function() {
    ...
    try {
      ...
      __log( "web api: Web workers are not defined, recording will not work.", e);
    }
  };
  ...
  var Recorder = function(cfg) {
    ...
    this.convertToMP3 = function() {
      ...
      fileReader.onload = function() {
        ...
        __log("web api: Converting to Mp3");
        ...
        encoderWorker.onmessage = function(e) {
          if (e.data.cmd == 'data') {
            ...
            __log("web api: Done converting to Mp3");
  ...
  var initRecorder = function() {
    if (global.audio_context) {
      console.log("web api: audio_context already ready");
      return;
    }
    try {
      // webkit shim
      ...
      __log('web api: Audio context set up.');
      __log('web api: navigator.getUserMedia ' + (navigator.getUserMedia ? 'available.' : 'not present!'));
    } catch (e) {
  ...
  var startUserMedia = function(stream) {
    if (global.audio_source) {
      console.log("web api: source already ready");
      return;
    }
    source = global.audio_source = audio_context.createMediaStreamSource(stream);
    __log('web api: Media stream created.');
    __log("web api: input sample rate " + source.context.sampleRate);
